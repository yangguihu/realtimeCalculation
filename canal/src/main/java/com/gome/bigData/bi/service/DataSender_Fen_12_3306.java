 ...
 */
public class DataSender_Fen_12_3306 {
    private static int getNum=0;
    public static void main(String args[]) {
        String zkServers = Configs.get("zkServers");
        String destination = Configs.get("DataSender_Fen_12_3306.destination");
        ...
        try {
            connector.connect();
            connector.subscribe("ccsdb\\.ccs_acct,ccsdb\\.ccs_merchandise_detail,rmpsdb_fen\\.tm_app_main,rmpsdb_fen\\.rule_output_bigdata");//修改的系统通过件数使用
            //connector.subscribe(".*\\..*");//监控数据库下所有库中所有表
            connector.rollback();
            int totalEmptyCount = Integer.parseInt(Configs.get("DataSender_Fen_12_3306.totalEmptyCount"));//控制无数据导致的空请求次数
            while (true) {
...
//                    eventType));
            ...
            //将数据发送到指定topic
            KafkaProducerUtils.send(Configs.get("DataSender_Fen_12_3306.topicName"),null,message);
        }
    }
